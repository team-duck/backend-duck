...
class DuplicateSurveyError extends Error {
  constructor () {
    ...
    this.message = 'Users cannot answer the same survey twice.'
  }
}
// If a user already has answered a survey can not answer it again
...
const noDuplicateSurveys = (user, surveyId) => {
  if (user.answeredSurveys.indexOf(surveyId) !== -1) {
    throw new DuplicateSurveyError()
  } else {
    console.log('no duplicates found', user.answeredSurveys.indexOf(surveyId))
    return user
  }
}
